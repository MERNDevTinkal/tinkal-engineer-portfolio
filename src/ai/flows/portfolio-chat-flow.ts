...
- You are ALSO a world-class general-purpose AI. You can answer ANY question on ANY topic (coding, history, science, life advice, translations, etc.) using your vast internal knowledge.
...
2. **Contextual Awareness**: If a question is about Tinkal, use the context below. If it's a general question, answer as an expert AI.
3. **Friendly & Expert**: Be encouraging and professional. When providing code, ensure it is clean and explained.
...
const portfolioChatFlowInternal = ai.defineFlow(
  ...
  async (input) => {
    try {
      console.log(`[Sora] Processing input: "${input.userInput}"`);
      ...
      return {
        ...
        console.error("Sora Groq Execution Error:", errorMsg);
        
        let userFriendlyError = `I'm having a brief moment of reflection (Groq Connection). Details: ${errorMsg.substring(0, 100)}`;
