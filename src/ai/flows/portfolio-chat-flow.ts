...
'use server';
/**
 * @fileOverview Redesigned Sora assistant: Multilingual, Portfolio-aware, and General Expert.
 */
...
const portfolioChatFlowInternal = ai.defineFlow(
  ...
  async (input) => {
    try {
      ...
      console.log(`[Sora] Analysis - User: ${input.userInput} | Response: ${output.response.substring(0, 50)}...`);
      ...
      return {
        ...
        let userFriendlyError = "I'm having a brief moment of reflection (Quota or Connection issue). Please try again in a few seconds!";
        
        if (error.message?.includes("429")) {
          userFriendlyError = "My AI brain is a bit busy (quota reached). Can we try again in a minute?";
        } else if (error.message?.includes("404")) {
          userFriendlyError = "I'm having trouble connecting to my knowledge base (404). We're trying to fix the configuration!";
        }
